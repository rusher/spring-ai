...
public class MariaDBVectorStore extends AbstractObservationVectorStore implements InitializingBean {
	...
	private static final String DEFAULT_COLUMN_EMBEDDING = "embedding";

	private static final String DEFAULT_COLUMN_METADATA = "metadata";
	...
	public List<Document> doSimilaritySearch(SearchRequest request) {
		...
		final String sql = String.format(
				"select %s, %s, %s, %s, vec_distance_%s(%s, ?) as distance "
						+ "from %s where vec_distance_%s(%s, ?) < ? %sorder by distance asc LIMIT ?",
				this.idFieldName, this.contentFieldName, this.metadataFieldName, this.embeddingFieldName, distanceType,
				this.embeddingFieldName, getFullyQualifiedTableName(), distanceType, this.embeddingFieldName,
				jsonPathFilter);

		logger.debug("SQL query: " + sql);

		return this.jdbcTemplate.query(sql, new DocumentRowMapper(this.objectMapper), embedding, embedding, distance,
				request.getTopK());
	}
