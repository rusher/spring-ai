 ...
 */
@Testcontainers
class MariaDBWithChatMemoryAdvisorIT {
	...
	private static void initStore(MariaDBVectorStore store) throws Exception {
	...
	private static MariaDBVectorStore createMariaDBVectorStoreUsingTestcontainer(EmbeddingModel embeddingModel)
			throws Exception {
		JdbcTemplate jdbcTemplate = createJdbcTemplateWithConnectionToTestcontainer();
		MariaDBVectorStore vectorStore = new MariaDBVectorStore.Builder(jdbcTemplate, embeddingModel).withDimensions(3) // match
			// embeddings
	...
	private static @NotNull JdbcTemplate createJdbcTemplateWithConnectionToTestcontainer() throws SQLException {
		MariaDbDataSource ds = new MariaDbDataSource();

		ds.setUrl("jdbc:mariadb://localhost:" + mariadbContainer.getMappedPort(5432) + "/" + schemaName);
		ds.setUser(mariadbContainer.getUsername());
		ds.setPassword(mariadbContainer.getPassword());
		ds.setUser("root");
	...
	void advisedChatShouldHaveSimilarMessagesFromVectorStore() throws Exception {
		...
		MariaDBVectorStore store = createMariaDBVectorStoreUsingTestcontainer(embeddingModel);
